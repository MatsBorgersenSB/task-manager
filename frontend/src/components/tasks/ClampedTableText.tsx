...
function clampClassName(maxLines: number): string {
  if (maxLines === 2) return "clamp-2 fade-clamp-2";
  if (maxLines === 5) return "clamp-5 fade-clamp-5";
  return `clamp-lines-${maxLines} fade-clamp-lines-${maxLines}`;
}

export type ClampedTableTextProps = {
  text: unknown;
  maxLines?: number;
  className?: string;
...
export default function ClampedTableText({
  text,
  maxLines = 2,
  className = "",
  emptyPlaceholder = "—",
}: ClampedTableTextProps) {
  ...
  const rawText = normalizeText(text).replace(/\r\n/g, "\n").trim();
  const hasContent = rawText.length > 0;
  const display = rawText || emptyPlaceholder;
  const showPopup = hasContent && (pinned || hovered);
  ...
  const showHoverPopup = useCallback(() => {
    ...
    setHovered(true);
    updatePopupPosition();
  }, [clearHideTimer, hasContent, updatePopupPosition]);
  ...
  if (!hasContent) {
    return (
      <span className={`text-[0.8125rem] text-primary/90 ${className}`}>
        {emptyPlaceholder}
      </span>
    );
  }
  ...
  return (
    <>
      <div
        ref={anchorRef}
        className={`relative block w-full min-w-0 cursor-pointer ${className}`}
        onClick={(event) => {
